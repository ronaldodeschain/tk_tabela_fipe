...
class Calculator(tk.Tk):
    def __init__(self):
        ...
        #change the icon of the aplication
        ...
        #cria os botões e adiciona eles no grid
        row = 1
        col = 0
        for button in buttons:
            button_action = lambda x=button: self.button_click(x)
            tk.Button(self,text=button,width=5,command=button_action).grid(
        ...
        #memory buttons
        tk.Button(self,text='M+',width=5,command=self.memory_add).grid(row=5,
                                                        column=1,padx=5,pady=5)
        tk.Button(self,text='M-',width=5,command=self.memory_subtract).grid(row=5,
                                                        column=2,padx=5,pady=5)
        tk.Button(self,text='MR',width=5,command=self.memory_recall).grid(row=5,
                                                        column=3,padx=5,pady=5)
        tk.Button(self,text='MC',width=5,command=self.memory_clear).grid(row=5,
                                                        column=4,padx=5,pady=5)

        #add clear e delete buttons
    ...
    def button_click(self,button):
        ...
        if button == '=':
            ...
            self.input_text.set(math.sin(math.radians(float(self.input_text.
                            get())))) #type:ignore
        elif button == 'cos':
            self.input_text.set(math.cos(math.radians(float(self.input_text.
                            get())))) #type:ignore
        elif button == 'tan':
            self.input_text.set(math.tan(math.radians(float(self.input_text.
                            get())))) #type:ignore
        elif button == 'sqrt':
            self.input_text.set(math.sqrt(float(self.input_text.get()))) #type:ignore
        else:
            #add the button's text to the input fiel
